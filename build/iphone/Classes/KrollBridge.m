...
NSString * project_mobileapp_catherinessssss$ModuleRequireFormat = @"(function(exports){"
...
@implementation project_mobileapp_catherinessssssObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_project_mobileapp_catherinessssss);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_project_mobileapp_catherinessssss gc];
}
...
{
	// create project_mobileapp_catherinessssss global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "project_mobileapp_catherinessssss" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_project_mobileapp_catherinessssss = [[project_mobileapp_catherinessssssObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_project_mobileapp_catherinessssss];
	
	NSString *_project_mobileapp_catherinessssssNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _project_mobileapp_catherinessssssNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_project_mobileapp_catherinessssss valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:project_mobileapp_catherinessssss$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.project_mobileapp_catherinessssss.kroll" 

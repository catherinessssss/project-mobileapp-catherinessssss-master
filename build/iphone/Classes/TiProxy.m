...
//Rare exceptions to indicate a bug in the _project_mobileapp_catherinessssss code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_project_mobileapp_catherinessssss = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_project_mobileapp_catherinessssss]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.project_mobileapp_catherinessssss.module"

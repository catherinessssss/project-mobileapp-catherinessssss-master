...
function Sync(method, model, opts) {
    function saveData(data, callback) {
        function iteration(data, i, queryList) {
            i = _.isNumber(i) ? i : 0;
            queryList = queryList || [];
            if (_.isUndefined(data[i])) return;
            queryList = _.isUndefined(data[i][model.deletedAttribute]) || true != data[i][model.deletedAttribute] ? -1 != _.indexOf(currentModels, data[i][model.idAttribute]) ? updateSQL(data[i], queryList) : createSQL(data[i], queryList) : deleteSQL(data[i][model.idAttribute], queryList);
            ++i < data.length ? _.defer(iteration, data, i, queryList) : _.defer(function() {
        ...
        if (_.isArray(data)) {
            ...
            data && data.length ? _.defer(iteration, data) : callback && callback();
        } else {
    ...
    function parseJSON(_response, parentNode) {
        ...
        _.isUndefined(parentNode) || (data = _.isFunction(parentNode) ? parentNode(data, opts) : traverseProperties(data, parentNode));
    ...
    switch (method) {
      case "create":
        ...
        apiCall(params, function(_response) {
            ...
            } else if (!_response.localOnly && params.disableSaveDataLocallyOnServerError) {
      ...
      case "read":
        ...
        apiCall(params, function(_response) {
            ...
            if (_response.success) if (304 != _response.code) {
                ...
                var data = parseJSON(_response, params.parentNode);
                params.localOnly ? successFn() : saveData(data, successFn);
            } else successFn(); else {
...
function _buildQuery(table, opts, operation) {
    ...
    if (opts.groupBy) {
        var group;
        group = _.isArray(opts.groupBy) ? opts.groupBy.join(", ") : opts.groupBy;
        sql += " GROUP BY " + group;
    }
    if (opts.orderBy) {
        var order;
